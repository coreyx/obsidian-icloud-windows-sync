...
- `stdin = subprocess.DEVNULL` -- see Post-Release Finding 8; a hidden console still gives the daemon a real, `isatty()`-true stdin it has no way to distinguish from an actual terminal.
- `stdout`/`stderr` are captured to `<logs_dir>\daemon_startup.log` (overwritten each launch) -- see Post-Release Finding 9. The daemon's own structured log file only exists after its first buffered flush, so a crash before that point would otherwise leave no trace anywhere.

### Log Rotation (Requirement 14)
The daemon writes one `sync_<timestamp>.log` file per process lifetime today, with no bound on how large it grows and no pruning except a one-time, count-based sweep (`log_retention`, existing) at the *next* startup. For a continuously-running daemon -- the normal case -- this means an unbounded-size active file and, if the Live Log window loads it wholesale (below), unbounded tray memory too.

- **`SyncConfig.max_log_size_mb`** (new field, `logging:` section, alongside `log_retention`; suggested default 10): checked in `SyncLogger.flush()` after each write. Once the current file's size crosses this threshold, the logger starts a new file exactly the way `init_log_file()` already names one (`sync_<new-timestamp>.log`) -- deliberately the *same* naming scheme a fresh process launch already produces, so nothing downstream (Live Log's "newest by mtime" discovery, `cleanup_old_logs()`'s pruning) needs to know rotation happened at all versus a restart. Track cumulative bytes written in memory between checks rather than `os.path.getsize()` on every flush.
- **`cleanup_old_logs()`** (existing) also runs from the periodic checkpoint in `sync_engine.py`'s main loop (the same one that already calls `log.flush()` every ~5s), not only once at startup -- otherwise a long-lived daemon that rotates many times between restarts would accumulate every rotated file for the life of that one run. Throttle this call to only re-run the directory sweep right after an actual rotation, not every checkpoint tick.

### Bounded Live Log (Requirement 14)
`log_viewer.py`'s `LogViewerWindow` currently loads a log file's entire contents into its Tk `Text` widget on open or file-switch (`_read_pos = 0`), then appends forever with no eviction -- memory tracks the file's size directly, unbounded for as long as the window stays open against an active daemon.

- **Tail-on-open**: instead of always seeking to byte 0, seek to `max(0, filesize - TAIL_BYTES)` (suggested ~256KB) on open or file-switch, discard the first (likely partial) line, and load from there. Bounds the *initial* load independent of rotation shipping -- relevant today, since an already-large file opened fresh has the same problem.
- **Scrollback cap**: after each poll's `_append()` inserts new lines, if the widget's line count exceeds `MAX_LINES` (suggested ~5000), delete from `"1.0"` to the excess line, same model as a terminal's scrollback buffer. Keeps memory bounded regardless of session length.
- Both are self-contained to `obsidian_sync_tray/log_viewer.py` -- no config or daemon-side dependency, and independent of whether the file-rotation half ships.
...
Requirement 14 adds one more `logging:` field, `max_log_size_mb: int = 10`, following the existing `log_retention` pattern exactly (dataclass field → `from_yaml`/`to_dict` round-trip → Options window Logging tab field + tooltip).
...
- Requirement 14's rotation/pruning piggybacks on timers that already exist (`flush()`'s existing call sites, the ~5s checkpoint) rather than introducing a new one; Live Log's scrollback cap only trims when a poll tick actually inserts new lines, not on its own timer.

## Post-Release Findings

Real installed-app usage (not just unit tests) surfaced several bugs the design above didn't anticipate. Each is fixed; documented here since none of them were predictable from code review alone.

1. **Stop permanently poisoned the next Start.** Neither the daemon nor `stop()` ever deletes `stop.request` on a graceful exit (by design — the daemon is exiting when it sees the file). Nothing implemented the "tray defensively deletes any stale one" line this doc already specified, so a leftover file from any prior run instantly killed every subsequent Start against the same `logs_dir` the moment the new process began polling for it. Fixed: `_launch()` now removes any pre-existing `stop.request` before spawning, for both Start and Run Once.
2. **Options (and the log viewer) opened but were never visible.** `OptionsWindow` called `self.transient(master)`, where `master` is the tray's permanently-hidden root. A `Toplevel` transient-for a withdrawn master stays stuck in the withdrawn WM state itself, *even after an explicit `deiconify()`* — confirmed with a minimal, isolated repro and via real OS window enumeration (`win32gui.EnumWindows`), not just Tk's own `.state()`, which reported "withdrawn" even to the constructing process. Fixed by removing the `transient()` call — it only makes sense against a real visible parent, which this app's root never is — and having `app.py`'s `_bring_to_front` call `deiconify()`/`lift()`/`focus_force()` on every open.
3. **Fresh installs hard-failed instead of prompting for setup.** No config file exists until the user creates one, but auto-start-on-launch (on by default) unconditionally tried to launch the daemon, which raised `FileNotFoundError` deep in `SyncConfig.from_yaml`. Fixed: `ProcessManager.ensure_config_exists()` creates a default config (blank vault paths, `history_dir`/`logs_dir` pre-filled inside the daemon's data folder) on first run, and `needs_setup()` — checked by Start, Run Once, and the auto-start path — redirects to Options instead of attempting a launch whenever `local_vault`/`icloud_vault` is blank.
4. **The Inno Setup uninstaller crashed with a runtime error dialog.** `WizardSilent()` is a *Setup*-only Pascal Script function; calling it from `CurUninstallStepChanged` (added to fix the silent-uninstall-hangs-on-a-custom-MsgBox issue from earlier testing) throws "Internal error: Cannot call 'WizardSilent' function during Uninstall" on every interactive uninstall. `UninstallSilent()` is the correct uninstall-context equivalent.
5. **Tests were writing into the real, shared `%APPDATA%` files.** `TrayApp.__init__` and `ProcessManager.__init__` default to a real `TrayLogger()`/real config path when no override is given; several tests constructed real instances without one, silently polluting the user's actual `tray.log` (and, separately, actually creating `C:\fake\config.yaml` + subdirectories on disk from a test fixture's literal placeholder path). Fixed by always injecting an isolated logger/`tmp_path`-based config path in tests that construct real instances.
6. **The Live Log window couldn't be closed while Options was also open.** `OptionsWindow` called `self.grab_set()`. Per Tcl/Tk's own documented `grab` semantics, an active *local* grab redirects all pointer events, application-wide, to the grabbing window -- with the log viewer as an independent sibling `Toplevel` (not a child of Options), this made its close button unresponsive to real clicks whenever Options was open at the same time. A synthetic `WM_CLOSE` posted directly to its window handle still worked (that bypasses pointer-event routing entirely), so this needed the Tcl/Tk documentation, not just an automated repro, to pin down conclusively; the regression test asserts the actual contract instead (`root.grab_current() is None` after opening Options). Fixed by removing `grab_set()` -- nothing in this app requires Options to be modal.
7. **The Live Log window showed plain, uncolored text, and was missing the startup banner and duplicate-scan lines entirely.** Two separate root causes. First, `write_to_file()` always called `strip_ansi()` before buffering, so the on-disk log carried no color information at all -- the Live Log window had nothing to color even if it tried. Second, in `__main__.py`, `DuplicateScanner.scan_and_clean()` ran *before* `SyncEngine.run()` called `logger.init_log_file()`, so every one of its log calls (and, separately, `logger.startup()`'s banner, which never called `write_to_file()` at all) hit `write_to_file()`'s "no log file yet" guard and were silently dropped from the file -- present on console, absent from disk. Fixed on three fronts: `init_log_file()` is now called in `__main__.py` before the duplicate scan (and made idempotent, since `SyncEngine.run()` still calls it too, for callers that invoke `run()` directly without going through `__main__.py`); `startup()` now writes each banner line to the file under `msg_type` `"BANNER"`/`"BANNER_TITLE"`; and `obsidian_sync_tray/log_viewer.py` reconstructs colors from each line's `[TYPE]` label via a `TYPE_COLORS` table matching `logger.py`'s own colorama scheme, rather than needing ANSI codes to survive in the file. One residual limitation: a handful of `TYPE` labels are reused by more than one logger method with different colors in the console (e.g. `"CLEAN"` is green from a successful duplicate scan but red from a partial-failure one; `"CONFIG"`, `"CONFLICT"`, `"FAILED"`, `"HISTORY"`, `"INFO"` similarly), and the on-disk line doesn't record which method wrote it -- `TYPE_COLORS` picks whichever color that label renders as on its common/every-run path, so the rare alternate case shows in the wrong color. Cosmetic only; the label and message text are unaffected either way.
8. **The daemon could hang forever, at 0% CPU, the instant it found any conflict/duplicate/`.tmp` file at startup.** Confirmed by hand against a real tray-launched daemon after pointing it at a vault with a genuine duplicate-named file: `tasklist` reported it "Not Responding" indefinitely, no log file was ever written (nothing reached `write_to_file()`'s 20-message auto-flush threshold), and Stop was powerless -- the daemon never got far enough into `SyncEngine.run()` to start watching for the stop file. Root cause: `duplicates.py`'s `scan_and_clean()` calls `input()` to ask about deleting found duplicates, a prompt written for a developer running the console daemon by hand. The tray launches the daemon with `CREATE_NO_WINDOW`, which still allocates a console-subsystem process a *real*, if hidden, console -- its stdin is a genuine terminal handle, `isatty()` included, so the daemon had no reliable way to tell nobody could ever type into it. Two-layer fix: `process_manager.py`'s `_launch()` now passes `stdin=subprocess.DEVNULL`, so any read of it hits EOF immediately regardless of what code tries to read it (the fix that actually resolves the hang, verified by re-running the exact `CREATE_NO_WINDOW` launch before and after against a real vault with a duplicate file present -- hung indefinitely before, exited cleanly after); `scan_and_clean()` also now checks `sys.stdin`/`isatty()` up front and skips the prompt with an explanatory log line when there's no real console, and catches `EOFError` alongside `KeyboardInterrupt` as defense in depth for any launch path that doesn't redirect stdin at all (e.g. Task Scheduler without explicit redirection).
9. **A daemon that fails to start at all left no trace anywhere.** Investigated a report of the daemon appearing to do nothing when started from the tray -- no new log file, tray showing idle again shortly after. Extensive direct testing (replicating the tray's exact command line, working directory, and even the real installed tray.exe launching the real installed daemon) never reproduced a failure; Norton's own detection log and quarantine folder were checked directly and showed nothing either. Root cause undetermined, but the investigation exposed a real, independent gap: the daemon's own structured log file is only created on its first buffered flush, so *any* crash before that point (bad arguments, config validation, an unhandled exception) has nowhere to go -- it prints to the hidden `CREATE_NO_WINDOW` console and is lost completely, leaving "it just didn't start" with no way to diagnose further. Fixed by having `_launch()` capture stdout/stderr unconditionally to `<logs_dir>\daemon_startup.log` (overwritten each attempt), so a future occurrence of this failure mode is actually diagnosable instead of a dead end.
