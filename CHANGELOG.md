...
- `max_log_size_mb` config setting (default 10): once the current log file crosses this size, the daemon starts a new one -- mid-run, not just at the next restart -- with the same `sync_<timestamp>.log` naming a fresh launch already produces, so nothing that discovers log files by name/mtime needs to treat it any differently. `cleanup_old_logs()`'s existing file-count pruning (`log_retention`) now also re-runs right after a rotation, not only at startup, so a daemon that never restarts still doesn't accumulate rotated files forever. The Live Log window now loads only the last ~256KB of a file on open instead of the whole thing, and caps itself at ~5000 displayed lines while tailing, discarding the oldest as new ones arrive -- between the two, neither disk usage nor the tray's memory should grow unbounded over an arbitrarily long uptime. See README's "Log rotation" section.
...
- Deleting an entire subfolder from the vault could leave the daemon endlessly logging "Still waiting for iCloud..." warnings, forever, for the folder's own path -- and, worse, silently fail to ever delete the files that had been inside it from iCloud/history at all. On Windows, `watchdog` can't stat a path that's already gone, so a whole-directory delete (or a move away from one) is sometimes misreported as a *file* event for the directory's own path instead of a directory event, and confirmed by hand: Windows sometimes never emits a separate delete event for the files that were inside the deleted folder at all -- only this one ambiguous folder-path event ever fires. `enqueue_path_event` now recognizes a `deleted`/`moved_from` event for a path that was never a real, hashed file (never a key in the sync-state cache, which a directory path never is) as this ambiguous case, and sweeps every file it knows was tracked under that path -- synthesizing a proper deleted event for any that are genuinely gone from disk, so they still propagate.
- The daemon could hang forever, at 0% CPU, the moment it found any conflict/duplicate/`.tmp` file at startup -- confirmed by hand against a real tray-launched daemon (indefinitely "Not Responding", no log file ever written since nothing reached the 20-message auto-flush threshold, and Stop powerless since it never got far enough to watch for the stop file). `CREATE_NO_WINDOW` still gives a console-subsystem process a real, if hidden, console, so `duplicates.py`'s `input()` y/N prompt was waiting on a keystroke nobody could ever type. The tray now launches the daemon with `stdin=subprocess.DEVNULL`, so any attempt to read it hits EOF immediately instead of blocking; `scan_and_clean()` also now checks for a real interactive console up front and skips the prompt with a clear log message when there isn't one, and catches `EOFError` alongside `KeyboardInterrupt` as defense in depth.
- The duplicate scanner flagged normally-titled notes ending in "(N)" (e.g. "Drive (2011).md", about something from 2011) as duplicates -- confirmed against a real vault where one such note was flagged three times over, once for each of local/iCloud/history, despite there being nothing actually duplicated. The iCloud-duplicate pattern only ever means anything when the un-suffixed sibling (`Drive.md`) also exists alongside it -- that's the actual signature of a real iCloud-created duplicate, since iCloud only appends "(N)" because the original was already there. `scan_and_clean()` now requires that sibling to exist before flagging a match. Separately, each finding is now labeled with which root (Local/iCloud/History) it came from -- the same relative path legitimately exists in more than one root for a fully-synced file, and without the label every root's copy looked like an identical, confusing repeat of the same "duplicate."
