...
- Deleting an entire subfolder from the vault could leave the daemon endlessly logging "Still waiting for iCloud..." warnings, forever, for the folder's own path. On Windows, `watchdog` can't stat a path that's already gone, so a whole-directory delete (or a move away from one) is sometimes misreported as a *file* event for the directory's own path instead of a directory event -- the daemon then tried to sync that bogus path as an ordinary file, "restoring" something that could never exist. `enqueue_path_event` now drops `deleted`/`moved_from` events for a path that was never a real, hashed file (i.e. never appeared in the sync-state cache), which a directory path never does.
